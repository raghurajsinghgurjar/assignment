...
abstract class REST_Controller extends CI_Controller {
	...
	protected function _client_id($clintId,$clientSeckey,$org_code){
		if(!empty($clintId) && !empty($clientSeckey)){
			$param = array();
			$param['adm_type'] = 'AD';
			$param['adm_client_id'] = $clintId;
			$param['adm_client_seckey'] = $clientSeckey;
			
			$this->load->model("admin/m_brands");
			$list = $this->m_brands->fetch($param);
	...
	function _get_contents($url,$param,$urlFlag=''){
		if(!empty($url) && !empty($param) && is_array($param)){
			...
			if(!empty($urlFlag) && ($urlFlag == 'vlt')){
				$urls = CURL_URL_VLT.$url.$where;
			}else{
				$urls = CURL_URL.$url.$where;
			}	
			//print $urls;die;
			//$detail = json_decode(file_get_contents($urls),true);
			$detail = json_decode($this->curl_get_file_contents($urls),true);
			if(!empty($detail) && is_array($detail) && !empty($detail['success']) && !empty($detail['data'])
